...
class ScenarioPasswordSecure extends JimpleDataflowFixture {
  ...
  behavior of
    """The program abstracts from a login scenario in which a user has a fixed number of login tries before the program
      ...
      |The class PasswordManager of the program stores the password, the number of unsuccesful login attempts, the
  ...
  "[Secure] The program" should "leak no information about the password to the user." taggedAs(HighConditional, ImplicitFlows) in {
    assertIsSecure(
      TaintSpec(
