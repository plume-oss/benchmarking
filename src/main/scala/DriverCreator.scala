...
object DriverCreator {
  ...
  val TMP_DIR: String =
    s"${System.getProperty("java.io.tmpdir")}${if (System.getProperty("java.io.tmpdir").endsWith(File.separator)) ""
    else File.separator}plume"
  ...
  def createOverflowDbDriver(config: java.util.Map[String, Any]): OverflowDbDriver = {
    ...
    val tmpFolder = new JavaFile(TMP_DIR)
    if (!tmpFolder.exists()) {
      tmpFolder.mkdirs()
    }
    if (config.getOrDefault("enabled", false) == true) {
      val storageLoc = config.getOrDefault("storage", s"$TMP_DIR${File.separator}cpg.bin").asInstanceOf[String]
      File(storageLoc).delete()
      DriverFactory
        .invoke(GraphDatabase.OVERFLOWDB)
        .asInstanceOf[OverflowDbDriver]
  ...
  def createJanusGraphDriver(config: java.util.Map[String, Any]): JanusGraphDriver =
    if (config.getOrDefault("enabled", false) == true) {
      DriverFactory
        .invoke(GraphDatabase.JANUS_GRAPH)
        .asInstanceOf[JanusGraphDriver]
  ...
  def createNeptuneDriver(config: java.util.Map[String, Any]): NeptuneDriver =
    if (config.getOrDefault("enabled", false) == true) {
      val driver = DriverFactory
        .invoke(GraphDatabase.NEPTUNE)
        .asInstanceOf[NeptuneDriver]
        .keyCertChainFile(
          config.getOrDefault("key_cert_chain_file", "src/main/resources/conf/SFSRootCAG2.pem").asInstanceOf[String]
        )
      ...
      config.get("hostnames").asInstanceOf[java.util.ArrayList[String]].forEach { host =>
        driver.addHostnames(host)
      }
      driver
    } else
      null
  ...
  def createTigerGraphDriver(config: java.util.Map[String, Any]): TigerGraphDriver =
    if (config.getOrDefault("enabled", false) == true) {
      DriverFactory
        .invoke(GraphDatabase.TIGER_GRAPH)
        .asInstanceOf[TigerGraphDriver]
  ...
  def createNeo4jDriver(config: java.util.Map[String, Any]): Neo4jDriver =
    if (config.getOrDefault("enabled", false) == true) {
      DriverFactory
        .invoke(GraphDatabase.NEO4J)
        .asInstanceOf[Neo4jDriver]
        .hostname(config.getOrDefault("hostname", "127.0.0.1").asInstanceOf[String])
