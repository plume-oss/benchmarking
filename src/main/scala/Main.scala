...
object Main extends App {
  ...
  val driverConfigs = YamlDeserializer.driverConfig("/driver_conf.yaml")
  println(driverConfigs)
  val datasetConfigs = YamlDeserializer.datasetConfig("/dataset_conf.yaml")
  println(datasetConfigs)

  PrettyPrinter.setLogger(logger)

  logger.info(s"Running ${experimentConfig.iterations} iterations of each benchmark")
  logger.info(s"Found ${datasetConfigs.configs.size} programs to benchmark against ${driverConfigs.configs.size} drivers.")
  logger.debug(s"The files are: ${datasetConfigs.configs.map(_.name).mkString(",")}")
//  drivers.foreach {
